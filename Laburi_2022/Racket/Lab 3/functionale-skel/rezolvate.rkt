...
;; O functie care returneaza o noua functie atunci cand este aplicata pe mai
;; putine argumente decat ii sunt necesare se numeste functie curry.
(define add-curry
  (lambda (x)
    (lambda (y)
      (+ x y))))
...
(define (sort-ins op)
  (lambda (L)
    (if (null? L)
        L
        ;; (ins-in-sorted op), dupa inauntru avem lambda(e L), unde e = elem din L
...
(define (foldr1 p acc L)
  (if (null? L)
      acc
      ;; operatia p se aplica dupa apelul recursiv
