...
(define (rev L)
  ;; Verific daca am lista nula
  (if (null? L)
      '() ;; Sau pot returna direct L din moment ce lista este nula
      (append (rev (cdr L)) (list(car L)))))

;; pe exemplu: rev '(5 1 4 8 7) <=> rev(5: '(1 4 8 7)) = rev '(1 4 8 7) ++ [5], deci rev (tail L) concatenat cu head(L)
...
(define (deep-rev L)
  (cond
    ...
    ((null? L)'())
    ;; Verific daca elementul curent este o lista
    ((list? (car L))
...
(define (palindromes-to-n n Bases)
  (cond
    ...
    ((all-palindromes? n Bases)
     ;; Daca da
     ;; adaug la inceput nr ce respecta conditia
     
     ;; nu stiu sa fac reverse pe cons 
     ;(reverse (cons n (palindromes-to-n (- n 1) Bases))))
...
(define (first-b-pal start b)
  (if (>= (count_num_bases start '(2 3 4 5 6 7 8 9 10)) b)
      start
      (first-b-pal (+ start 1) b)))

(define (count_num_bases n L)
  (cond
    ((null? L) 0)
    ;; Iau pe rand fiecare numar din L, care reprezinta o baza
    ;; si verific daca n in baza x (x apartine L) este palindrom
    ((palindrome? (num->base n (car L))) (+ 1 (count_num_bases n (cdr L))))
    (else (count_num_bases n (cdr L)))))
...
(define (longest-palindrome n)
   (list->num (find-longest-part (num->base n 10) (length (num->base n 10)) '())))

;; Aceasta functie auxiliara vreau sa intoarca o lista
(define (find-longest-part L len result)
  (cond
    ;; len = lungimea listei L
    ;; Daca lungimea listei rezultat este mai mare decat cea a lui L, am gasit cea mai lunga portiune
    ((< len (length result)) result)
    ;; Prefixul listei sa fie palindrom && daca lungimea listei rezultat inca nu a ajuns mai mare ca len
    ;; Daca and returneaza #t, apelez recursiv functia pe lista ramasa (tail)
    ((and (palindrome? (take L len)) (> len (length result))) (find-longest-part (cdr L) (length (cdr L)) (take L len)))
    ;; Daca se returneaza #f, apelez tot pe lista ramasa, insa la result actualizez si lungimea listei in care caut
    (else (find-longest-part (cdr L) (length(cdr L)) (find-longest-part L (- len 1) result)))))
