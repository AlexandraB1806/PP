...
-- VAR2: pattern matching
reverseList2 :: [a] -> [a]
-- Caz de baza: Daca lista este nula => lista rezultat este si ea nula
reverseList2 [] = []
-- Caz general: lista are elemente
...
-- VAR5: foldl
reverseList5 :: [a] -> [a]
-- foldl primeste 3 parametrii: o functie, un acumulator si o lista.
-- Functia o construim ca fiind una lambda. Primeste ca parametrii un
-- acumulator (o lista) si un element ce urmeaza a fi adaugat in lista => operatia este cons
-- acc e o lista vida la inceput !
reverseList5 l = foldl (\ acc elem -> elem : acc) [] l

-- VAR6: foldr
reverseList6 :: [a] -> [a]
reverseList6 l = foldr (\ elem acc -> acc ++ [elem]) [] l
...
computeLength getLineSegment getStartPoint getEndPoint =
    let segment = getLineSegment
        ...
        stopX = fst stop
        stopY = snd stop
    in sqrt (((startX - stopX) ** 2) + ((startY - stopY) ** 2))

-- cu where
computeLength2 :: ((Double, Double), (Double, Double)) 
            -> (((Double, Double), (Double, Double)) -> (Double, Double)) 
            -> (((Double, Double), (Double, Double)) -> (Double, Double)) -> Double
computeLength2 getLineSegment getStartPoint getEndPoint = sqrt $ ((startX - stopX) ** 2) + ((startY - stopY) ** 2)
    where
