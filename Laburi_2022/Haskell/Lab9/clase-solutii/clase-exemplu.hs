...
type Result = (TeamName, Int, Int)

data Team = T (TeamName, [Result])

points :: Team -> Int
points (T team) = zipWith (\ x y  -> if x > y then res + 3 else res + 1) marked_goals received_goals where
                    marked_goals = map (\ (x, y, z) -> y) (snd team)
                    received_goals = map (\ (x, y, z) -> z) (snd team)
