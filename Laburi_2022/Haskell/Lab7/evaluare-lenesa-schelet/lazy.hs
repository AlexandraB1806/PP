...
buildPairsStringLength l = zip l (map length l)
...
setUnion :: Eq a => [a] -> [a] -> [a]
{-
Din multimea B trb sa elinmin elementele comune cu multimea A.
Deci:
    1) Extrag elem comune cu multimea A;
    2) Aplic diferenta intre multimea B si multimea elementelor comune;
    3) Concatenez A cu rez.
-}
...
{-
    ...
    ce vom aplica formula de mai sus in mod iterativ, rezultatul va fi din ce în ce
...
newtonRaphsonSolve x_old atol g dg = result
  where
    flux = newtonRaphson x_old g dg
    vals = zip (tail flux) flux
    -- perechea (dont care, result) adaugata in alt dont care;  actual type `[(Double, Double)]' !!!
    -- deci o lista de perechi
...
-- Inlocuiesc functia si derivata functiei cu cele specifice met babiloniene
