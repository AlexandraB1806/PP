...
myConcat([],L,L).
% se pune direct in concluzie H1 atat la prima lista, cat si la rez
...
% myReverse(_,_):- false.
myReverse([], []).
myReverse([H|T], Sol) :- myReverse(T, TempSol), myConcat(TempSol, [H], Sol).
...
% 0! = 1
factorial(0, 1).
% daca n > 0, pot apela recursiv factorial(n - 1). Se face pattern matching iar F1 = 1.??
...
descendantOf(X, Y) :- arc(Y, X) ; arc(Interm, X), descendantOf(Interm, Y).
...
drum(N1, N2, [N1|T]) :- descendantOf(X, N1), descendantOf(N2, X), drum(X, N2, T).
drum(N1, N2, [N1|T]) :- descendantOf(N1, X), drum(X, N2, T).
...
% cost(_, _, _):- false.
cost(N, N, 0).
cost(N, DescDir, 1) :- descendantOf(DescDir, N).
cost(N, SuccDir, -1) :- descendantOf(N, SuccDir).
cost(N1, N2, C) :- descendantOf(N2, X), descendantOf(X, N1), cost(X, N2, N), C is N + 1.
cost(N1, N2, C) :- descendantOf(N1, X), cost(X, N2, N), C is N - 1.
