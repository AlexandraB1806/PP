...
; (get-nth-tuple Fs tuple) este o functie care il asteapta pe n
(define (get-nth-tuple Fs tuple)
  (λ(n)
    (apply-functional-transformations Fs tuple)))

; FUNCTII AUXILIARE PENTRU ULTIMELE 3 TASKURI
...
(define get-nth-ppt-from-matrix-transformations
  (λ(n)
    ((get-nth-tuple (convert-transformations-for-triplets n) '(3 4 5)) n)))
...
(define get-nth-quadruple
  (λ(n)
    ((get-nth-tuple (convert-transformations-for-tuplets n) '(1 1 2 3)) n)))
...
; Functia construieste o noua lista care reprezinta TPP-ul final.
